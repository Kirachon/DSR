...
const Spinner: React.FC<LoadingProps> = ({
  ...
  if (variant === 'pulse') {
    return (
      <div
        className={cn(loadingVariants({ variant: 'pulse', size }), className)}
      />
    );
  }

  return (
    <div className={cn(loadingVariants({ variant, size, color }), className)} />
  );
};
...
export const Skeleton: React.FC<{
  ...
  return (
    <div className={cn('animate-pulse', className)}>
      ...
      {button && <div className='mt-4 h-10 bg-gray-300 rounded w-24' />}
    </div>
  );
};
