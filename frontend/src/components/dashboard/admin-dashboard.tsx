...
interface SecurityAlert {
  id: string;
  type: 'login_failure' | 'suspicious_activity' | 'policy_violation' | 'system_breach';
...
const securityAlerts: SecurityAlert[] = [
  ...
  },
  {
    ...
    severity: 'high',
    description: 'Unusual data access pattern detected for user ID 12847',
  ...
  },
  {
    ...
    severity: 'low',
    description: 'Password policy violation - weak password detected',
    timestamp: '1 day ago',
    resolved: true,
  },
];
    ...
    <div className="space-y-6">
      ...
      {criticalAlerts.length > 0 && (
        <Alert variant="error" title="Critical Security Alerts">
          {criticalAlerts.length} high-priority security alert{criticalAlerts.length > 1 ? 's' : ''} require{criticalAlerts.length === 1 ? 's' : ''} immediate attention.
          <Link href="/admin/security/alerts" className="font-medium underline ml-1">
            View alerts
          </Link>
        </Alert>
      )}
            ...
            <Link href="/admin/security">
              <div className="flex items-center p-3 border border-gray-200 rounded-lg hover:border-primary-300 hover:shadow-md transition-all duration-200 cursor-pointer">
                ...
                <div className="ml-3">
                  ...
                  <p className="text-sm text-gray-600">Monitor security alerts and policies</p>
                </div>
              </div>
            </Link>
              ...
              <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center space-x-3">
                  <div className={`w-3 h-3 rounded-full ${
                    service.status === 'online' ? 'bg-success-500' :
                ...
                <div className="text-right">
                  <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                    service.status === 'online' ? 'bg-success-100 text-success-800' :
        ...
        <div className="space-y-4">
          {securityAlerts.slice(0, 5).map((alert) => (
            <div key={alert.id} className={`flex items-start space-x-3 p-3 rounded-lg border ${
              ...
              alert.severity === 'high' ? 'bg-warning-50 border-warning-200' :
